...
class Toolset:
    ...
    def post_method(self, user_id, manager, start_date, end_date, leave_type, note):
        ...
        if end_dtime == start_dtime:
            ...
            while period != "0" and period != "1" and period != "2":
                period = class_chat_input(
                    "Invalid input. Type only the number respectively:\n0. All day\n1. Only the morning\n2. Only the afternoon\n", self.msg)
    ...
    Is this information correct? Type 1 to submit, type 0 if you want to tell the bot to edit the form.\n'''
        ...
        while user_confirm != "1" and user_confirm != "0":
            user_confirm = class_chat_input(
                "Invalid input. Type 1 to submit, type 0 if you want to tell the bot to edit the form.\n", self.msg)
    ...
    def dayoff_allow(self):
        response = requests.get(url + f'/api/User/dayoff-data?email={self.email}').json()['data']
        return response['dayOffAllow']
    ...
    def delete_leave_applications(self, target: str = "Default"):
        applis = requests.get(url + f'/api/LeaveApplication/{get_userId(self.email)}').json()['data']
