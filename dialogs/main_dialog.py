        ...
        if step_context.result:
            ...
            if token_response and token_response.token:
                ...
                not self.bot.agent_session[me_info['mail']][1]:
                    hrm_token = self.bot.get_token(me_info['mail'])
                    ...
                    t1 = threading.Thread(target=run_agent, args=(self, msg, me_info['mail'], agent_arg, hrm_token,), daemon=True)
                    t1.start()
...
def run_agent(obj: MainDialog, query, email, agent_type, hrm_token):
    obj.bot.agent_session[email][1] = True
    if agent_type == 1:
        print("Running agent 1...")
        obj.bot.agent[email].run1(query, hrm_token)

    else:
        print("Running agent 2...")
        obj.bot.agent[email].run2(query, hrm_token)
