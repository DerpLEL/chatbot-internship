...
# os.environ["AZURE_COGNITIVE_SEARCH_API_KEY"] = "73Swa5YqUR5IRMwUIqOH6ww2Bm3SveLv7rDmZVXtIAzSeBjEQe9
...
Input: How many articles are there in chapter 9?
Output: chapter IX, chương IX
<|im_end|>
{context}
<|im_start|>user
Input: {question}
...
Output: policy
Input: How many articles are there in chapter 8?
Output: policy
...
Do not answer if the sentence is a question. Only output the category and nothing else.
Input: {question}
<|im_start|>assistant
Output:"""
...
"""
    ...
    def add_label_history(self, user_msg, ai_msg):
        hist = self.label_history

        hist.append({'user': user_msg, 'AI': ai_msg})

    def add_keyword_history(self, user_msg, ai_msg):
        hist = self.keyword_history
    ...
    def chat_public(self, query):
        keywords = self.keywordChain({'question': query, 'context': self.get_keyword_history()})['text']
        print(f"Query: {query}\nKeywords: {keywords}")
        self.add_keyword_history(query, keywords)
    ...
    def chat_private(self, query):
        label = self.classifier_chain({'question': query, 'context': self.get_label_history()})['text']
        print(f"Label: {label}")
        self.add_label_history(query, label)
